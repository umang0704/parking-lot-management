...
public class OperatorService {
    ...
    public Operator checkIfOperatorExist(Long operatorId, ParkingLot parkingLot) throws ApplicationException {
        Optional<Operator> operator = operatorRepository.findByIdAndParkingLot_Id(operatorId,parkingLot.getId());
        if(operator.isPresent()) return operator.get();
        else throw new ApplicationException(ResponseCode.PL_400402,"Operator with id: "+operatorId+"not found.");
    }
}
