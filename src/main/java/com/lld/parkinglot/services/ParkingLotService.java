...
public class ParkingLotService {
    ...
    public ParkingLotDetailsDto createDto(ParkingLotCreationDto parkingLotCreationDto) {
        ...
        IntStream.range(0, parkingLotCreationDto.getOperatorCount()).forEach(operatorIndex -> {
        ...
        ParkingLotDetailsDto parkingLotDetailsDto = new ParkingLotDetailsDto(
                ...
                (int) parkingSpotRepository.countByParkingFloor_IdInAllIgnoreCase(parkingFloorRepository.findIdByParkingLot_Id(parkingLot.getId())),
                operatorDtoList,
                (int) gateRepository.getGatesByTypeAndParkingLotId(parkingLot.getId(), GateType.ENTRY.toString()),
                (int) gateRepository.getGatesByTypeAndParkingLotId(parkingLot.getId(), GateType.EXIT.toString())
        );

        return parkingLotDetailsDto;
    }

    private void createParkingFloorAndRelatedParkingSpot(ParkingLotCreationDto parkingLotCreationDto, ParkingLot parkingLot) {
        IntStream.range(0, parkingLotCreationDto.getFloors()).forEach(parkingFloorIndex -> {
            ...
            IntStream.range(0, parkingLotCreationDto.getSpotsOnFloor()).forEach(parkingSpotOnFloor -> {
    ...
    private void createGatesForParkingLot(ParkingLotCreationDto parkingLotCreationDto, ParkingLot parkingLot) {
        IntStream.range(0, parkingLotCreationDto.getEntryGates()).forEach(entryGateIndex -> {
            Gate gate = new Gate();
        ...
        IntStream.range(0, parkingLotCreationDto.getExitGates()).forEach(exitGateIndex -> {
            Gate gate = new Gate();
            gate.setParkingLot(parkingLot);
            gate.setGateStatus(GateStatus.CLOSED);
    ...
    public ParkingLot checkIfParkingLotExist(Long parkingLotId) throws ApplicationException {
        ...
        if (parkingLot.isPresent()) return parkingLot.get();
            ...
            throw new ApplicationException(ResponseCode.PL_400401, "Parking Lot with id: " + parkingLotId + "  not found!");
    }
}
